...
import { DEFAULT_PARAMS } from './CalculationParams';
import calculateWage, { MonthWageInfo } from './calculateWage';
import Time from '../Time';
import HourEntry from '../hourEntry/HourEntry';
import * as Big from 'big.js';

function getFirstPersonFirstMonthEveningWage(monthlyWages: MonthWageInfo[]) {
...
describe('calculateWage', () => {
  describe('when evening compensation end is before start', () => {
    ...
    it('calculates evening compensation from first day', () => {
      const entries = [
      ...
      expect(getFirstPersonFirstMonthEveningWage(result)).toEqual("2.00")
    })

    it('calculates evening compensation from second day', () => {
      const entries = [
      ...
      expect(getFirstPersonFirstMonthEveningWage(result)).toEqual("3.00")
    })

    it('calculates evening correctly when multiple entries during evening', () => {
      const entries = [
  ...
  describe('when evening compensation start is before end', () => {
    ...
    it('calculates evening compensation correctly', () => {
      const entries = [
    ...
    it('calculates evening compensation correctly when only part of shift is on evening', () => {
      const entries = [
        new HourEntry("foo", 1, moment("2016-01-01 22:30"), moment("2016-01-02 02:00")), // 0.5h during evening
      ]

      const result = calculateWage(entries, params)

      // evening compensation from 0.5h => 1.50
      expect(getFirstPersonFirstMonthEveningWage(result)).toEqual("0.50")
    })
    ...
    it('calculates evening compensation from second day', () => {
      ...
      // evening compensation from 3h => 3.00
      expect(getFirstPersonFirstMonthEveningWage(result)).toEqual("6.00")
    })
  })
})
